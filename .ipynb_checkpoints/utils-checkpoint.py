...
#        for line in lines:
#            fo.write(line + '\n')
def eval_lab(model, test_dataloader, epoch):
    
    pred_np, lab_np = [], []
    model.eval()
    with torch.no_grad():
        for b_tok, b_mask, b_sour_mask, b_targ_mask, b_sent_mask, b_lab in tqdm(test_dataloader):
            b_pred_prob = model(b_tok, b_sour_mask, b_targ_mask, token_type_ids=b_sent_mask, attention_mask=b_mask)
            b_pred = torch.argmax(b_pred_prob, dim=-1).squeeze()
            pred_np.append(b_pred.numpy().tolist())
            lab_np.append(b_lab.numpy().tolist())
    p, r, f, _ = precision_recall_fscore_support(b_lab, b_pred, average='weighted')
    print('Precision %.2f%%, Recall %.2f%%, F1 %.2f' % (100 * p, 100 * r, 100 * f))
    print('Accuracy: %.4f%%' % accuracy_score(lab_np, pred_np))
